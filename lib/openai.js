...
},
{
  ...
  function: {
    ...
    description: "Atualiza os indicadores do painel principal sem criar gasto financeiro.",
...
async function handleToolCall(toolCall, userId) {
 ...
 case 'registrar_movimento_painel': {
  ...
  switch (args.movement_type) {
    ...
    case 'prejuizo':
    case 'caixa':
      settingKey = 'current_balance';
      break;

    case 'faturamento':
      settingKey = 'monthly_revenue';
      break;
  }
  ...
  let currentValue = Number(existing?.value || 0);

  if (args.movement_type === 'lucro') {
    currentValue += amount;
  }

  if (args.movement_type === 'prejuizo') {
    currentValue -= amount;
  }

  if (args.movement_type === 'faturamento') {
    currentValue += amount;
  }
