...
console.log('DEBUG CARD:', {
  originalUsed: card.used_amount,
  installmentAmount,
  argsAmount: args.amount,
  installments: args.installments
});
    
    const {
  data: transacoesCartao,
  error: fetchCardTransactionsError
} = await supabase
  .from('financial_transactions')
  .select('*')
  .eq('card_id', card.id);

if (fetchCardTransactionsError)
  throw fetchCardTransactionsError;

const totalUsado = transacoesCartao.reduce((sum, t) => {
  const valor = Number(
