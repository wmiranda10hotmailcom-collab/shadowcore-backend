...
amount: { type: "number", description: "Valor do gasto" },
          ...
          installments: {
  type: "number",
  description: "Quantidade de parcelas. Se o usuário falar parcelado em X vezes, preencher com X."
},
...
cardholder_name: {
  type: "string",
  description: "Nome do titular, nome do banco OU os últimos 4 dígitos do cartão. Exemplos: João, Maria, Nubank, Santander, Inter, 1234, final 1234."
},
...
return data.find(card => {
  ...
  (numeros && last4.includes(numeros))
);
}) || null;
}
...
const installmentAmount =
  installments > 1
    ? Math.abs(Number(args.amount)) / installments
...
installments: installments,
  ...
  const novoValor =
    ...
    Number(valorAdicionar || 0);

  const { error: cardError } = await supabase
    .from('cards')
    .update({
      used_amount: novoValor
    })
    .eq('id', card.id);
...
if (args.installments && args.installments > 1) {
  ...
  const newFixedCost = {
    id: crypto.randomUUID(),
    name: args.title || args.category || 'Parcelamento',
    amount: installmentAmount,
    currency: args.currency || 'BRL',
    nextChargeDay: new Date().getDate(),
    installments: args.installments,
    totalAmount: Math.abs(args.amount),
    cardName: card?.cardholder_name || null,
    autoGenerated: true
  };

  const updatedValue = {
    ...currentValue,
    fixedCosts: [...fixedCosts, newFixedCost]
  };
...
- Receitas aumentam saldo e também aumentam o valor de "Já faturado" no painel principal.
...
- Para entradas de dinheiro, faturamento, receita operacional ou entrada de caixa:
  use movement_type = "faturamento"

- Apenas quando o usuário quiser definir o saldo atual/manual:
  use movement_type = "caixa"
